...
const CommunityUpdateListLayer = () => {
    ...
    const [data, setData] = useState({
        ask: Array.from({ length: 20 }).map((_, i) => ({
            id: i + 1,
            chapterName: ['Chennai Central', 'Mumbai South', 'Delhi West', 'Bangalore East', 'Hyderabad North', 'Kolkata Metro', 'Pune City', 'Ahmedabad GIDC', 'Jaipur Pink', 'Lucknow Nawabs', 'Chandigarh Royal', 'Coimbatore Elite', 'Madurai Star', 'Trichy Titans', 'Salem Warriors', 'Erode Kings', 'Vellore Fort', 'Nellore Coast', 'Vizag Port', 'Kochi Spice'][i],
    ...
    const handleDelete = () => {
        const newData = { ...data };
        newData[activeTab] = newData[activeTab].filter(item => item.id !== itemToDelete.id);
        setData(newData);
        ...
        <>
            ...
            <Modal show={showDeleteModal} onHide={handleClose} centered>
                <Modal.Body className="text-center p-5">
                    ...
                    <p className="text-secondary-light mb-4">Do you want to delete this item? This action cannot be undone.</p>
